...
typedef enum {
   ...
   ACTION_TILEMAP2,     // Quick build (tilemap ordering) optimize tiles and generate mapping
...
int basetile = 0;
int dump_pal = 0;
const char *palfilename = NULL;

// Function prototypes
int build_tilemap(const char *, const char *);
int build_tilemap2(const char *, const char *, const char *);
...
int main(int argc, char **argv) {
   ...
   for (int curr_arg = 1; curr_arg < argc; curr_arg++) {
      ...
      if (scan_ok && arg[0] == '-') {
         ...
         // Quick build?
            ...
            action = action == ACTION_DEFAULT ?
                     ACTION_TILEMAP2 : ACTION_TOOMANY;
         else if (!strcmp(arg, "-s") || !strcmp(arg, "--sprite"))
            action = action == ACTION_DEFAULT ?
                     ACTION_SPRITE : ACTION_TOOMANY;

         // New options
         else if (!strcmp(arg, "-o") || !strcmp(arg, "--offset")) {
             curr_arg++;
             if(curr_arg >= argc) {
                 fprintf(stderr, "Error: Number nor specified for base option.\n");
                 errcode = 1;
             }
             arg = argv[curr_arg];
             if(arg[0] == '$') {
                 basetile = strtol(&arg[1], NULL, 16);
             } else if(arg[0] == '0' && (arg[1] == 'X' || arg[1] == 'x')) {
                 basetile = strtol(&arg[2], NULL, 16);
             } else {
                 basetile = strtol(arg, NULL, 10);
             }
         }
         else if (!strcmp(arg, "-p") || !strcmp(arg, "--dumppal")) {
            dump_pal = 1;
            curr_arg++;
            if(curr_arg >= argc) {
                fprintf(stderr, "Error: No filename spacified for dumppal option.\n");
                errcode = 1;
            }
            palfilename = argv[curr_arg];
         }
      ...
      else if (action == ACTION_TILEMAP2 && out2filename == NULL)
          out2filename = arg;
   ...
   if (show_help) {
      printf("Usage:\n"
             "  %s -b <batchfile>\n"
             "  %s -t <infile> <outfile>\n"
             "  %s -m <infile> <outfile> <outfile2>\n"
             "  %s -s <infile> <outfile>\n"
             "\n"
             "Options:\n"
             "  -b or --batch ..... Batch build (default)\n"
             "  -t or --tilemap ... Quick build, tilemap tile order\n"
             "  -m or --map ....... Like -t but deduplicate tiles and output mapping\n"
             "  -s or --sprite .... Quick build, sprite tile order\n"
             "  -4 or --4bpp ...... Output 4bpp tiles (quick build)\n"
             "  -1 or --1bpp ...... Output 1bpp tiles (quick build)\n"
             "  -o or --offset <num> ... Specify offset and tile attributes in mapping\n"
             "                           Number may be prepended with 0x or $ for hex\n"
             "  -p or --dumppal <file> . Dump palette to specified file\n"
   ...
   switch (action) {
      ...
      case ACTION_TILEMAP:
         ...
         break;
      // Quick build (tilemap ordering)
      case ACTION_TILEMAP2:
          set_output_format(format);
          errcode = build_tilemap2(infilename, outfilename, out2filename);
          break;
...
int build_tilemap2(const char *infilename, const char *outfilename, const char *out2filename) {
    // Load input bitmap
    Bitmap *in = load_bitmap(infilename);
    if (in == NULL)
        return ERR_OPENINPUT;
    ...
    FILE *out2 = fopen(out2filename, "wb");
    if (out2 == NULL) {
        destroy_bitmap(in);
        fclose(out);
        return ERR_OPENOUTPUT;
    ...
    // Parse the bitmap as a massive tilemap
    set_map_offset(basetile);
    int errcode = generate_map(in, out, out2, 0, 0,
                                in->width >> 3, in->height >> 3, 0);
